...
- `GITHUB_SYNC_TOKEN_SECRET` (recommended when auth is on) — encrypts workspace PAT
  links at rest (falls back to `NUXT_SESSION_PASSWORD`, then a weak dev default).
...
- **Compose parity matters for P2 rollout.** Keep `.env.example`,
  ...
  `BETTER_AUTH_*`, `GITHUB_CLIENT_*`, `GITHUB_SYNC_TOKEN_SECRET`, `GITHUB_APP_*`,
  `GITHUB_WEBHOOK_SECRET`, `VAULT_READONLY`).
